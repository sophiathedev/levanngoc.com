defmodule LevanngocWeb.SeoImageLive.GeoTag do
  ...
  def mount(_params, _session, socket) do
    {:ok,
     ...
     |> allow_upload(:images, accept: ~w(.jpg .jpeg .png .svg .gif), max_entries: 10, auto_upload: true)
  ...
  def handle_event("validate", params, socket) do
    ...
    has_non_jpeg = Enum.any?(entries, fn entry ->
  ...
  defp process_geotag(file_path, params, original_filename) do
    ...
    if params.compress_after_geotag do
      System.cmd("mogrify", ["-quality", to_string(params.image_quality), file_path], stderr_to_stdout: true)
    end
  ...
  defp set_metadata_with_exiftool(file_path, metadata) do
    case System.find_executable("exiftool") do
      nil ->
        ...
        IO.puts("Exiftool not found. Falling back to mogrify for metadata (may not set all tags correctly).")
  ...
  def render(assigns) do
    ...
    <div class="w-full p-4">
      ...
      <form phx-change="validate" phx-submit="add_geo_tag" class="space-y-6">
        <!-- Upload Section -->
        <div class="relative w-full p-10 flex flex-col items-center justify-center gap-2 border-2 border-dashed border-base-300 rounded-lg hover:bg-base-200/20 transition-colors" phx-drop-target={@uploads.images.ref}>
           ...
           <p class="text-sm text-base-content/60">Hoặc kéo & thả file vào đây (Định dạng file: .JPG; .JPEG; .PNG; .SVG; .GIF)</p>
          ...
          <div class="card-body p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div class="form-control w-full">
                <label class="label pt-0"><span class="label-text font-medium">Latitude (vĩ độ)</span></label>
                <input type="text" name="latitude" value={@latitude} placeholder="Nhập vĩ độ" class="input input-bordered w-full" />
              </div>
              <div class="form-control w-full">
                <label class="label pt-0"><span class="label-text font-medium">Longitude (kinh độ)</span></label>
                <input type="text" name="longitude" value={@longitude} placeholder="Nhập kinh độ" class="input input-bordered w-full" />
              </div>
            </div>
          </div>
        </div>
        ...
        <%= if @has_non_jpeg do %>
          <div role="alert" class="alert alert-warning shadow-sm">
            ...
            <span>Một số file không phải định dạng JPG/JPEG. Các trường Title, Subject, Keywords và Comments sẽ bị vô hiệu hóa.</span>
          </div>
        <% end %>
        <div class="card bg-base-100 shadow-sm border border-base-200">
          <div class="card-body p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
               ...
               <div class="form-control w-full">
                 ...
                 <input type="text" name="title" value={@title} disabled={@has_non_jpeg} class={"input input-bordered w-full bg-base-200/30 #{if @has_non_jpeg, do: "opacity-50 cursor-not-allowed"}"} />
               </div>
               <!-- Subject -->
               <div class="form-control w-full">
                 <label class="label pt-0"><span class="label-text font-medium">Subject</span></label>
                 <input type="text" name="subject" value={@subject} disabled={@has_non_jpeg} class={"input input-bordered w-full bg-base-200/30 #{if @has_non_jpeg, do: "opacity-50 cursor-not-allowed"}"} />
               </div>
               <!-- Keywords -->
               <div class="form-control w-full">
                 <label class="label pt-0"><span class="label-text font-medium">Keywords</span></label>
                 <input type="text" name="keywords" value={@keywords} disabled={@has_non_jpeg} placeholder="..." class={"input input-bordered w-full bg-base-200/30 #{if @has_non_jpeg, do: "opacity-50 cursor-not-allowed"}"} />
               </div>
               <!-- Secondary Keywords -->
               <div class="form-control w-full">
                 <label class="label pt-0"><span class="label-text font-medium">Keywords phụ</span></label>
                 <input type="text" name="secondary_keywords" value={@secondary_keywords} disabled={@has_non_jpeg} placeholder="..." class={"input input-bordered w-full bg-base-200/30 #{if @has_non_jpeg, do: "opacity-50 cursor-not-allowed"}"} />
               </div>
               <!-- Comments -->
               <div class="form-control w-full">
                 <label class="label pt-0"><span class="label-text font-medium">Comments</span></label>
                 <input type="text" name="comments" value={@comments} disabled={@has_non_jpeg} class={"input input-bordered w-full bg-base-200/30 #{if @has_non_jpeg, do: "opacity-50 cursor-not-allowed"}"} />
               </div>
               <!-- Author -->
               <div class="form-control w-full">
                 <label class="label pt-0"><span class="label-text font-medium">Author</span></label>
                 <input type="text" name="author" value={@author} class="input input-bordered w-full bg-base-200/30" />
               </div>
               <!-- Copyright -->
               <div class="form-control w-full">
                 <label class="label pt-0"><span class="label-text font-medium">Copy Right</span></label>
                 <input type="text" name="copyright" value={@copyright} class="input input-bordered w-full bg-base-200/30" />
               </div>
               <!-- Date Taken -->
               <div class="form-control w-full">
                 <label class="label pt-0"><span class="label-text font-medium">Date Taken</span></label>
                 <input type="date" name="date_taken" value={@date_taken} class="input input-bordered w-full bg-base-200/30" />
               </div>
            </div>
          </div>
        </div>
        ...
        <div class="card bg-base-100 shadow-sm border border-base-200">
          <div class="card-body p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
               <div class="form-control w-full">
                 <label class="label pt-0"><span class="label-text font-medium">Camera Manufacturer:</span></label>
                 <input type="text" name="camera_manufacturer" value={@camera_manufacturer} placeholder="Nhập nhà sản xuất" class="input input-bordered w-full" />
               </div>
               <div class="form-control w-full">
                 <label class="label pt-0"><span class="label-text font-medium">Camera Model</span></label>
                 <input type="text" name="camera_model" value={@camera_model} placeholder="Tên máy" class="input input-bordered w-full" />
               </div>
               <div class="form-control w-full">
                 <label class="label pt-0"><span class="label-text font-medium">Application Name</span></label>
                 <input type="text" name="application_name" value={@application_name} placeholder="Tên ứng dụng" class="input input-bordered w-full" />
               </div>
            </div>
          </div>
        </div>
                ...
                <%= for {label, name, checked} <- [
                  ...
                  <div class="form-control border-b border-dashed border-base-200 last:border-none">
                    <label class="label cursor-pointer flex justify-between items-center py-3">
                      ...
                      <input type="checkbox" name={name} value="true" class="toggle toggle-primary toggle-sm" checked={checked} />
                    </label>
                  </div>
                <% end %>
                ...
                <div class="mt-6">
                  ...
                  <div class="form-control border-b border-dashed border-base-200">
                    <label class="label cursor-pointer flex justify-between items-center py-3">
                      ...
                      <input type="checkbox" name="compress_after_geotag" value="true" class="toggle toggle-primary toggle-sm" checked={@compress_after_geotag} />
                    </label>
                  </div>
                  ...
                  <div class="form-control py-4">
                     <div class="flex items-center gap-4">
                        ...
                        <input type="range" min="0" max="100" value={@image_quality} name="image_quality" class="range range-primary range-xs flex-1" />
                     </div>
                  </div>
                </div>
             </div>
        ...
        <div class="flex gap-2 justify-end mt-4">
           <button type="button" phx-click="clear" class="btn btn-outline min-w-[150px] bg-white hover:bg-gray-100 hover:text-gray-800 border-base-300">
